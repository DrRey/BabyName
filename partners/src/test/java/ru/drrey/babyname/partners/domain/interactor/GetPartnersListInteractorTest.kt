...
class GetPartnersListInteractorTest {

    private val testDispatcher = TestCoroutineDispatcher()

    private val partnersRepository: PartnersRepository = mock()
    private val getUserId = { flowOf(userId) }
    private val interactor = GetPartnersListInteractor(partnersRepository, getUserId)
    ...
    fun testFlow() {
        runBlockingTest {
            interactor.buildFlow(null).collect { assert(it == partnersList) }
        }
    }
