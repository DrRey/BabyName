...
class SavePrimaryColorInteractorTest {

    private val testDispatcher = TestCoroutineDispatcher()

    private val themeRepository: ThemeRepository = mock()
    private val interactor = SavePrimaryColorInteractor(themeRepository)

    @Before
    fun setup() {
        Dispatchers.setMain(testDispatcher)
        whenever(themeRepository.savePrimaryColor(any())).thenReturn(flowOf(Unit))
    }
