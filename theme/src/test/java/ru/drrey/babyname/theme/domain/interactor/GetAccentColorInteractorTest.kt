...
class GetAccentColorInteractorTest {

    private val testDispatcher = TestCoroutineDispatcher()

    private val themeRepository: ThemeRepository = mock()
    private val interactor = GetAccentColorInteractor(themeRepository)
    private val result = 111

    @Before
    fun setup() {
        Dispatchers.setMain(testDispatcher)
        whenever(themeRepository.getAccentColor()).thenReturn(flowOf(result))
    }
